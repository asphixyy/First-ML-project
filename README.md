...
It addresses class imbalance using SMOTE and implements multiple classifiers (Decision Tree, Random Forest, and XGBoost) to find the best model. 
The final model is serialized and deployed within a custom real-time inference loop.
...
For telecom providers, identifying high-risk customers before they leave is critical to reducing retention costs and maintaining revenue stability.

This project outlines a complete machine learning workflow:
* **Exploratory Data Analysis (EDA)** to understand churn patterns.
* **Pre-processing** categorical variables via Label Encoding and handling data type formatting.
...
* **Serialization & Predictive Inference** to classify churn risk for new customers.
...
1. **Clone the repository**:
   ```bash
   git clone https://github.com/asphixyy/customer-churn-prediction.git
